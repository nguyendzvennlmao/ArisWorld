...
public class ArisCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
        ...
        if (act.equals("create")) {
            ...
            p.sendMessage(color("&#facc15[ᴀʀɪꜱ] ᴆᴀɴɢ xᴀ̂ʏ ᴅᴜ̛̣ɴɢ " + type + "..."));
    ...
    public List<String> onTabComplete(CommandSender s, Command c, String l, String[] a) {
        ...
        if (a.length == 2) return Bukkit.getWorlds().stream().map(World::getName).filter(name -> name.toLowerCase().startsWith(a[1].toLowerCase())).collect(Collectors.toList());
        if (a.length == 3 && a[0].equalsIgnoreCase("create")) return Arrays.asList("desert", "overworld", "spawn").stream().filter(st -> st.startsWith(a[2].toLowerCase())).collect(Collectors.toList());
