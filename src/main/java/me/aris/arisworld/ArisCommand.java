...
public class ArisCommand implements CommandExecutor, TabCompleter {
    ...
    public ArisCommand(ArisWorld plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender s, Command c, String l, String[] a) {
        ...
        Player p = (Player) s;
        if (a.length < 2) {
            p.sendMessage(color("&#ff0812[ᴀʀɪꜱ] /aw <create|tp|delete> <name> <type>"));
            return true;
        }
        ...
        if (act.equals("create")) {
            ...
            p.sendMessage(color("&#facc15[ᴀʀɪꜱ] ᴆᴀɴɢ xᴀ̂ʏ ᴅᴜ̛̣ɴɢ ᴛʜᴇ̂́ ɢɪᴏ̛́ɪ " + type));
            ...
            else p.sendMessage(color("&#ff0812[ᴀʀɪꜱ] ᴡᴏʀʟᴅ ᴋʜᴏ̂ɴɢ ᴛᴏ̂̀ɴ ᴛᴀ̣ɪ!"));
        } else if (act.equals("delete")) {
            World wd = Bukkit.getWorld(name);
            if (wd != null) Bukkit.unloadWorld(wd, false);
            deleteFile(new File(Bukkit.getWorldContainer(), name));
            List<String> worlds = plugin.getConfig().getStringList("worlds");
            worlds.removeIf(st -> st.startsWith(name + ":"));
            plugin.getConfig().set("worlds", worlds);
            plugin.saveConfig();
    ...
    public List<String> onTabComplete(CommandSender s, Command c, String l, String[] a) {
        if (a.length == 1) return Arrays.asList("create", "tp", "delete").stream().filter(st -> st.startsWith(a[0].toLowerCase())).collect(Collectors.toList());
        if (a.length == 2) return Bukkit.getWorlds().stream().map(World::getName).filter(nm -> nm.toLowerCase().startsWith(a[1].toLowerCase())).collect(Collectors.toList());
        if (a.length == 3 && a[0].equalsIgnoreCase("create")) return Arrays.asList("desert", "flat", "overworld", "nether", "the_end").stream().filter(st -> st.startsWith(a[2].toLowerCase())).collect(Collectors.toList());
